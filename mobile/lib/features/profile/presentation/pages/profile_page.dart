...
import '../../../../features/progress/presentation/widgets/skill_radar_chart.dart';
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // ── Gamification ───────────────────────────────────
            _buildGamificationSection(context).animate().fadeIn(delay: 200.ms).slideY(begin: 0.05),
            const SizedBox(height: 12),

            // ── Change Password ────────────────────────────────
            _buildChangePasswordSection(context).animate().fadeIn(delay: 250.ms).slideY(begin: 0.05),
            const SizedBox(height: 12),

            // ── 2FA ───────────────────────────────────────────
            _buildTwoFactorSection(context, user).animate().fadeIn(delay: 300.ms).slideY(begin: 0.05),
            const SizedBox(height: 12),

            // ── Account Info ───────────────────────────────────
            _buildAccountInfoSection(context, user).animate().fadeIn(delay: 350.ms).slideY(begin: 0.05),
      ...
      child: Consumer(
        builder: (context, ref, _) {
          ...
          return progressAsync.when(
            ...
            error: (err, stack) => Text('Could not load stats: $err'),
            data: (progress) {
              return Column(
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      _StatCol('Simulations', '${progress.totalSimulations}'),
                      _StatCol('Win Rate', '${progress.overallWinRate.toStringAsFixed(1)}%'),
                      _StatCol('Avg Return', '${progress.averagePnlPct.toStringAsFixed(2)}%'),
                    ],
                  ),
                  const SizedBox(height: 16),
                  SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      ...
                      onPressed: () => context.push(AppConstants.achievementsRoute),
                    ),
                  ),
                ],
              );
